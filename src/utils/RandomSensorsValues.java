...
public class RandomSensorsValues {
    ...
    public ArrayList<Float> generate() {
        ...
        if ((this.seriousCondition && this.changeTendency < 0.99)
            ...
            this.sensors.add(
                    (float) RandomUtil.generateInt(CRITICAL_RESPIRATORY_FREQUENCY_START, CRITICAL_RESPIRATORY_FREQUENCY_END)
            );
            this.sensors.add(
                    RandomUtil.generateFloat(CRITICAL_BLOOD_OXYGENATION_START, CRITICAL_BLOOD_OXYGENATION_END)
            );
            this.sensors.add(
                    (float) RandomUtil.generateInt(CRITICAL_BLOOD_PRESSURE_START, CRITICAL_BLOOD_PRESSURE_END)
            );
            this.sensors.add(
            ...
            this.sensors.add(
                    (float) RandomUtil.generateInt(NORMAL_RESPIRATORY_FREQUENCY_START, NORMAL_RESPIRATORY_FREQUENCY_END)
            );
            this.sensors.add(
                    RandomUtil.generateFloat(NORMAL_BLOOD_OXYGENATION_START, NORMAL_BLOOD_OXYGENATION_END)
            );
            this.sensors.add(
                    (float) RandomUtil.generateInt(NORMAL_BLOOD_PRESSURE_START, NORMAL_BLOOD_PRESSURE_END)
            );
            this.sensors.add(
                    (float) RandomUtil.generateInt(NORMAL_HEART_RATE_START, NORMAL_HEART_RATE_END)
            );
        }
